  ...
  return isDesktop ? (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{product ? "Detail Product" : "Add New Product"}</DialogTitle>
        </DialogHeader>
    ...
    <Drawer open={isOpen} onClose={onClose}>
      <DrawerContent>
        <DrawerHeader>
          <DrawerTitle>{product ? "Detail Product" : "Add New Product"}</DrawerTitle>
        </DrawerHeader>
        <ProductForm sessions={session} product={product} onSubmit={handleSubmit} />
      </DrawerContent>
    </Drawer>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="number" {...field} disabled={true} onChange={(e) => field.onChange(Number(e.target.value))} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
